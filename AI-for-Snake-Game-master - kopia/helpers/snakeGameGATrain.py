...
class SnakeGameGATrain(SnakeGameGATest):
	...
	def game_over(self):
		...
		if self.cur_chrom == self.chroms_per_gen:
			...
			print(current_time, self.num_generations, self.high_score, average_game_score, average_fitness)

			self.game_scores = []

			#Write data about this generation to ga_data.txt
			file = open("GAdata.txt", "a")
			file.write("Time of save" + current_time +"\n")
	...
	def calc_fitness(self):
		...
		#If the frames since the last fruit was eaten
		if self.frames_since_last_fruit >= 200:
			...
			frame_score = self.frames_alive - self.frames_since_last_fruit - 2 
